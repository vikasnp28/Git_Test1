Configuration SharePointConfig
{
    ...
    Node ($AllNodes.Where{$_.Role -eq 'CM'}).NodeName
    {
        <#
        ...
        <#
        xWaitForADDomain WaitForADDomainOnCM
        {
            DomainName = $Node.DomainToJoin
            DomainUserCredential = $DomainJoinCredential
            RebootRetryCount = $Node.RebootRetryCountCM
            RetryCount = $Node.RetryCountCM
            RetryIntervalSec = $Node.RetryIntervalSecCM
            DependsOn = '[DnsServerAddress]SetDNSServerAddress'
        }
        
        $SplitDomain = $Node.DomainToJoin.split('.')[0]
        Computer ConfigMgrDomainJoin
        {
            Name = $Node.NodeName
            ...
            DependsOn = '[xWaitForADDomain]WaitForADDomainOnCM'
        }
